...
void loop() {
    ...
    for (int i = 0; i < SEND_SIZE; i++) {
      sendData[i] = '\0';  // 初期値としてヌル文字を代入する
    }
    for (int i = 0; i < SEND_SIZE/COLUMN; i++) {
      char largeNumberCharArray[COLUMN];
      itoa(i+counter*SEND_SIZE/COLUMN, largeNumberCharArray, 10); // 10進数として変換
      strcat(sendData, largeNumberCharArray); // 既存のchar型配列に追加
      sendData[strlen(sendData)] = '\n'; // スペース文字を追加
    } 
    counter = counter+1;
    if (counter == 12000/(SEND_SIZE/COLUMN)){
      break;
    }
    Serial.println("end");
